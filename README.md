...
The `docker-compose.yaml` file will create a MySQL Database, a simple web based db ui (localhost:8080), and the canary server.

Make sure to configure the db initial credentials on the `docker-compose.yaml` file and also to update it on your `config.lua`. You can use `mysql` as the hostname for the database because it will be resolved to the MySQL container inside Docker's network. The MySQL container will automatically run the `schema.sql` script once the db is created. This will only run on the first time.
